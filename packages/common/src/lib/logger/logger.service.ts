// This file is a modified version of the Logger from @nestjs/common
// https://github.com/nestjs/nest/blob/master/packages/common/services/logger.service.ts
import { Injectable, Optional } from '@nestjs/common'
import { clc, darkGray, yellow } from './utils/cli-colors.util'
...
export class Logger implements LoggerService {
  ...
  private static printMessage(
    ...
    const pidMessage = color(`${clc.magentaBright(clc.bold(`> ${this.app}`))} ${clc.darkGray(process.pid)} `)
    const contextMessage = context ? darkGray(`[${context}] `) : ''
